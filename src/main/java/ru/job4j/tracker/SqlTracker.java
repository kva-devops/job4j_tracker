...
public class SqlTracker implements Store {
    ...
    public List<Item> sortDescendId() {
        List<Item> result = new ArrayList<>();
        try (PreparedStatement statement = cn.prepareStatement("select * from items order by id desc")) {
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    result.add(new Item(
                            resultSet.getString("name"),
    ...
    public List<Item> sortDescendName() {
        List<Item> result = new ArrayList<>();
        try (PreparedStatement statement = cn.prepareStatement("select * from items order by name desc")) {
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    result.add(new Item(
                            resultSet.getString("name"),
    ...
    public List<Item> sortAscendName() {
        List<Item> result = new ArrayList<>();
        try (PreparedStatement statement = cn.prepareStatement("select * from items order by name asc")) {
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    result.add(new Item(
                            resultSet.getString("name"),
